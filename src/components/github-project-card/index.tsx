...
const GithubProjectCard = ({
  ...
  const renderProjects = () => {
    return githubProjects.map((item, index) => (
      <a
        className="card shadow-md card-sm bg-base-100 cursor-pointer transform transition-transform duration-300 hover:scale-105"
        href={item.html_url}
        key={index}
        onClick={(e) => {
          e.preventDefault();
        ...
        <div className="flex justify-between flex-col p-8 h-full w-full">
          <div>
            <div className="flex items-center truncate">
              <div className="card-title text-xl tracking-wide flex text-base-content opacity-60">
                <MdInsertLink className="my-auto" />
                <span>{item.name}</span>
              </div>
            </div>
            <p className="mb-5 mt-1 text-base-content text-base opacity-75">
              {item.description}
            </p>
          </div>
